...
import type { WritePayload } from "../../src/types.js";
...
// Semantic distractors — thematically similar to needles but with different facts
  ...
  for (const agent of agents) {
    for (let i = 0; i < memoriesPerAgent; i++) {
      results.push({
        ...
        agent,
        meta: { sourceAgent: agent, agentRole: agent === "reasoning" ? "reasoning" : "proactive" },
      });
    }
  }
  return results;
