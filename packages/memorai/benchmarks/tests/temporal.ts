...
// Tests: can we retrieve memories within specific time ranges?

import { Memorai, MemoryAdapter } from "../../src/index.js";
import { OllamaEmbeddingService } from "../../src/embeddings/ollama.js";
import { generateTemporalMemories } from "../lib/data.js";
...
export async function runTemporalBenchmark(): Promise<BenchmarkResult> {
  ...
  for (let q = 0; q < QUERIES; q++) {
    ...
    // Score: what fraction of expected memories were retrieved?
    const expectedIds = new Set(expected.map((m) => m.expectedTime));
    ...
    const recall =
      expected.length > 0
        ? retrievedInRange.length / expected.length
        : 1;
