...
async function scenario() {
  ...
  triggers.nodes.forEach((n) =>
    console.log(`  - ${n.payload.summary} (${n.hierarchy.level})`),
  )
  ...
  console.log(
    `  ${all.filter((n) => n.hierarchy.level === 'segment').length} segments`,
  )
  console.log(
    `  ${all.filter((n) => n.hierarchy.level === 'atomic_action').length} atomic actions`,
  )
  console.log(
    `  ${all.filter((n) => n.hierarchy.level === 'event').length} events`,
  )
