...
export class RetrievalEngine {
  ...
  async retrieve(query: RetrievalQuery): Promise<RetrievalResult> {
    ...
    const avgScore =
      result.nodes.length > 0
        ? result.nodes.reduce((sum, n) => sum + (n._score ?? 0), 0) /
          result.nodes.length
        : 0
    ...
    switch (query.strategy) {
      ...
      case 'inferential':
        ...
        results = results.map((n) => {
          ...
          if (n.hierarchy.childrenIds && n.hierarchy.childrenIds.length > 2)
            boost *= 1.2
          return { ...n, _score: n._score * boost }
        })
        break
  ...
  private keywordScore(node: MemoryNode, text: string): number {
    ...
    if (node.payload.description?.toLowerCase().includes(lowerText))
      score += 0.3
    const tagHits = node.payload.tags.filter((t) =>
