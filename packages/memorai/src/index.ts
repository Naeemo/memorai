...
export class Memorai {
  ...
  async write(
    ...
    const boostedSalience =
      payload.payload.salienceScore * profile.writePolicy.salienceBoost
  ...
  async retrieve(query: RetrievalQuery): Promise<RetrievalResult> {
    const mergedQuery: RetrievalQuery = {
      ...
      traversalOrder:
        query.traversalOrder ?? this.agentProfile.readPolicy.defaultTraversal,
      agentRole: query.agentRole ?? this.agentProfile.role,
    }

    if (mergedQuery.text && !mergedQuery.embedding) {
      mergedQuery.embedding = await this.config.embedding.embed(
        mergedQuery.text,
      )
    }
  ...
  async delete(id: string, cascade = false): Promise<void> {
    ...
    if (node?.hierarchy.parentId) {
      ...
      if (parent?.hierarchy.childrenIds) {
        parent.hierarchy.childrenIds = parent.hierarchy.childrenIds.filter(
          (cid) => cid !== id,
        )
